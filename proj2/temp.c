...
int categoryExecute(SHELLCMD *t)
{
    ...
    int saved_stdout = dup(1); //store stdout for restore
    int saved_stdin = dup(0); //store stdin for restore
    ...
    else if(typeCmd == CMD_PIPE) //Pipe exec
    {
        int pipe1[2];
            ...
            dup2(saved_stdout, 1); //Restore
            dup2(saved_stdin, 0); //Restore
    ...
    dup2(saved_stdout, 1); //Restore incase it was changed
    dup2(saved_stdin, 0); //Restore incase it was changed
